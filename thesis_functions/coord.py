...
def fit_line(x, y):
    ...
    y : np.ndarray
        Array containing the y-values of the data.

    Returns
    -------
    coef : list or float
        The coefficient(s) of the linear regression
    intercept : float
        Intercept of the linear regression
    """
    # The algorithm is unstable for a high slope, so we check if there is very
    # little variation on the x-axis compared to the y-axis (giving a near
    # vertical slope). If so, we
    covy = np.cov(y)
    relvar = np.cov(x) / covy if covy != 0 else 1.

    if relvar < 1e-2:

        # Swap the axes
        results = linregress(y, x)
        coef, intercept = results.slope, results.intercept

        if coef == 0:
            raise ValueError("Vertical line")

        # And then swap the axes for the results
        intercept = -intercept/coef
        coef = 1/coef
    else:
        # Otherwise, we just continue on normally
